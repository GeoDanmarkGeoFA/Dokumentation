...
For at sikre en ensartet registrering af adresser i GeoFA er der *ikke mulighed* for at indtaste alle adresseoplysninger. I stedet skal man registere en relventat nøgle til enten ***DAR*** eller ***vejkode***
...
- **adr_id**
  ...
  Udfyldes med den UUID som angiver den adresse som faciliteten i GeoFA ligger på/ved og hvis udfyldt bruges til auomatisk beregning af andre felter.
...
- **postnr**
  ...
  Udfyldes med det 4-ciferede nummer for postdistrikt.
...
Se den komplette spcifikation af felterne i GeoFA specifikationen. Felterne er, hvis det er bestemt således, tilknyttet de enkelte lag i GeoFA og fungerer da som beskrevet.
...
I eksemplet herover skal man altså kopiere teksten "0a3f5089-25f0-32b8-e044-0003ba298018" og indsætte den i feltet "adr_id" i GeoFA.

> [!NOTE]
> Når feltet **adr_id** får en værdi vil en systemmæssig natlig kørsel indsætte værdierne ud fra et opslag i DAR (Danmarks Adresseregister).
...
En lang række af temaerne i kan man udfylde et felt med navnet *postnr*. Feltet udfyldes med et 4 cifret tal som svarer til det postnmummer som hører til adressen. Postnumre vedligeholdes ultimativt i DAGI (se https://confluence.sdfi.dk/pages/viewpage.action?pageId=10616969) og replikeres til GeoFA hvor det benyttes til tilknytning af værdien til feltet *postnr_by*

Hvis man indsætter en (lovlig) værdi i feltet *postnr*, så vil feltet *postnr_by* automatisk indeholde navnet postdistriktet.

Hvis man ønsker at fjerne tilknytningen til postdistriktet, så skal værdien af feltet *postnr* slettes (sættes = NULL).
...
Felterne indeholder en teksttuel oversættelse af det geometriske punkt som er registreret for faciliteten i GeoFA Felterne er i DMS-format (grader, minutter, sekunder + retning) og kan benyttes af de bruger-systemer der ikke kan håndtere geometrien som den ellers leveres af GeoFA. Felteterne udfyldes automatisk hver nat af systemet.
...
Feltet indeholder kommunenummert (uden foranstillet 0) for den kommune som punktet er beliggende i. Feltet udfyldes automatisk hver nat af systemet på baggrund af en analyse mellem facilitetens punkt og DAGI-kommunegrænserne.
